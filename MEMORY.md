...
- Tests: `pytest` from repo root (no Blender required for parser/export tests). Targeted modules: `tests/test_template_parse.py`, `tests/test_purge_resolve.py`, `tests/test_gcode_edit.py`, `tests/test_package_imports.py`, `tests/test_export_transitions.py`, `tests/test_export_with_real_template.py`.
- **G-code compose / template parse packages:** `blender_addon.template_parse` (`markers.py`, `toolchange.py`, `purge_towers.py`, `parse.py`, `types.py`) and `blender_addon.export` (`compose.py`, `purge_resolve.py`, `motion.py`, `gcode_edit.py`, `types.py`). Public imports unchanged: `from blender_addon.template_parse import parse_bambu_template, ParsedTemplate` and `from blender_addon import export`. Purge lookup: `export.resolve_purge_for_layer`; Z shift: `export.rebase_swap_z` (was private helpers on monolithic modules).
...
- **Add-on version:** bump only the **`"version": (x, y, z)`** tuple in [blender_addon/__init__.py](blender_addon/__init__.py) `bl_info` (must stay a literal — Blender uses `ast.literal_eval` on `bl_info`; `"version": ADDON_VERSION` breaks discovery). [blender_addon/version.py](blender_addon/version.py) reads that tuple via AST for `ADDON_VERSION` / panel `ADDON_VERSION_STRING`. In the **3D Viewport**, press **N** → sidebar **Slicer** tab → the panel title is **`Slicer (vX.Y.Z)`** after reload.
